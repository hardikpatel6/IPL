...
const TABLE_NAME = process.env.TABLE_NAME_MATCHES as string;
const BATCH_SIZE = 25;
...
async function seedMatchTable() {
  const chunks = chunkArray(AllMatchData, BATCH_SIZE);

  for (let i = 0; i < chunks.length; i++) {
    console.log(`Uploading Match batch ${i + 1}/${chunks.length}`);
    await batchWrite(chunks[i]);
  }

  console.log("✅ Match table seeded successfully");
}

seedMatchTable().catch(console.error); 